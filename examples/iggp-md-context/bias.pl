%% BEST PROGRAM:
%% => 11 literals: 5 vars
%% next_value(B):-c_player(C),c5(B),c_pressButton(D),does(C,D)
%% next_value(B):-my_succ(D),my_true_value(D),c_player(C),c_noop(E),does(C,E)

%% TP: 8, FN: 0, TN: 46, FP: 0
%% => 6 literals: 2 vars
%% next_value(5):-does(player,pressButton).
%% next_value(A):-succ(B,A),true_value(B),does(player,noop).
