%% (base) ➜  filter popp exs.pl modes1.pl bk.pl
%% f(A,B) :- empty(A),empty(B).
%% f(A,B) :- cons2(C,D,A),odd(C),f(D,B).
%% f(A,B) :- even(D),cons2(D,C,A),f(C,E),cons1(D,E,B).
%% python3 /Users/andrew/icloud/code/popper/popper.py exs.pl modes1.pl bk.pl  23.43s user 0.17s system 99% cpu 23.621 total
