...
% ********** SOLUTION **********
% Precision:1.00 Recall:1.00 TP:7 FN:0 TN:201 FP:0 Size:7
% drop(A,B,C,D):- room(B,D),gripper(C,D),ball(A,D),goal_at(A,B,D),at_robby(B,D),carry(A,C,D).
% ******************************
...
type(drop,(ball,room,gripper,ex_id)).
