...
def popper(settings, tester, bkcons):
    ...
    # ----------------------------
    # Former methods -> nested helpers
    # ----------------------------
    def check_redundant_literal(prog):
        if len(prog) > 1:
            return [], False
        ...
        for i in range(len(body)):
            ...
            out.extend(check_redundant_literal_aux(new_body, redundant_literal, allsat1, allsat2, depth=0))
    ...
    def check_redundant_literal_aux(body, literal, allsat1, not_all_sat1, depth):
        out = []
        ...
        if hash(prog_key) in seen_allsat:
            return out

        h_, b = remap_variables((None, body | frozenset([literal])))
        if hash(b) in seen_allsat:
            return out

        seen_allsat.add(hash(prog_key))
        seen_allsat.add(hash(b))
        ...
        if not connected(body | frozenset([literal])):
            for new_body in combinations(body, len(body) - 1):
                ...
                out.extend(check_redundant_literal_aux(new_body, literal, allsat1, not_all_sat1, depth + 1))
            return out
        ...
        if not settings.has_valid_directions((None, body)):
            return out

        if not tester.is_literal_redundant(body, literal):
    ...
    for size in range(2, max_size + 1):
        ...
        while True:
            ...
            with settings.stats.duration('check_reducible1'):
                xs, pruned_smaller = check_redundant_literal(prog)
                if pruned_smaller:
            ...
            if (not add_spec) and (not pruned_more_general) and settings.datalog and (not settings.recursion_enabled) and num_neg > 0:
                with settings.stats.duration('check_reducible2'):
                    bad_prog = check_neg_reducible(prog)
                    if bad_prog:
                        add_spec = True
                        pruned_more_general = True
                        if settings.showcons:
