...
class Generator:
    ...
    def constrain(self, tmp_new_cons):
        ...
        for xs in tmp_new_cons:
            ...
            if con_type == Constraint.GENERALISATION:
                ...
                # for x in xs:
                #     print('gen', x)
                self.new_ground_cons.update(xs)
            elif con_type == Constraint.SPECIALISATION:
                print('spec')
                con_size = None
                if self.settings.noisy and len(xs)>2:
                    con_size = xs[2]
                # self.new_ground_cons.update(self.build_specialisation_constraint3(con_prog, size=con_size))
            elif con_type == Constraint.UNSAT:
                print('unsat')
                pass
                # self.new_ground_cons.update(self.unsat_constraint2(con_prog))
    ...
    def make_rule_handle2(self, rule):
        head, body = rule
        body_literals = sorted(body, key=lambda atom: atom[0])
        handle = ''.join(f'{literal.predicate}{"".join(map(str,literal.arguments))}' for literal in [head] + body_literals)
        return handle
    ...
    def build_specialisation_constraint3(self, prog, size=None):
        ...
        if handle in self.seen_handles:
            ...
            self.seen_handles.update(self.build_seen_rule2(base, False))
    ...
    def redundancy_constraint1(self, prog):
        ...
        head, body_ = rule
        self.seen_handles.update(self.build_seen_rule2(rule, False))
        for ruleid in range(self.settings.max_rules):
